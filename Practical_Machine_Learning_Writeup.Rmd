...
***
# Introduction
As part of the "Practical Machine Learning" MOOC course at Coursera, this project uses data from accelerometers on the belt, forearm, arm, and dumbell of 6 participants to build a machine learning algorithm to predic the "classe" variable in the data set. 

***
# Background
(An excerpt from the course project information) Using devices such as Jawbone Up, Nike FuelBand, and Fitbit it is now possible to collect a large amount of data about personal activity relatively inexpensively. These type of devices are part of the quantified self movement – a group of enthusiasts who take measurements about themselves regularly to improve their health, to find patterns in their behavior, or because they are tech geeks. One thing that people regularly do is quantify how much of a particular activity they do, but they rarely quantify how well they do it. In this project, your goal will be to use data from accelerometers on the belt, forearm, arm, and dumbell of 6 participants. They were asked to perform barbell lifts correctly and incorrectly in 5 different ways. More information is available from the website here: [http://groupware.les.inf.puc-rio.br/har](http://groupware.les.inf.puc-rio.br/har), under the section on the **Weight Lifting Exercise Dataset**.
...
Just in case, let us check the collinearity between potential numeric predictors.
...
By partitioning the training set data (training, from **pml-training.csv**) to 2 subsets : training.train, and training.test, we would then be able to compare the out-of-sample accuracy/error rates among the choices of models, before deciding on the final model, which would then be assessed by the testing set (**pml-testing.csv**) 
...
```{r echo=TRUE, message=TRUE}
Modelfit1
confusionMatrix.train(Modelfit1)  ##--> accuracy at 0.9985. Or, equivalently, the error rate was 0.0015
```
...
Use this model to make prediction on training.test to validate the error rate. 
...
confusionMatrix1  	##--> accuracy at 0.9988.  Or, equivalently, the error rate was 0.0012. Not very different from that in the traning.train.
...
The out-of-sample accuracy was an impressive 0.9988, or eqivalently, the error rate was 0.0012 !
...
```
```{r, echo = TRUE, message = TRUE}
confusionMatrix.train(Modelfit2)    ####--> accuracy at 0.9963.  Or, equivalently, the error rate wais 0.0037
```
...
confusionMatrix2  	##--> accuracy at 0.9964, or, equivalently, the error rate at 0.0036
...
The out-of-sample accuracy was also an impressive 0.9964, or, equivalently, the error rate was 0.0036!

Both of Random Forests and Generalized Boosted Model had close to perfect out-of-sample accuracy/error rate.
